...
**Multi-worker deploy (`burrow-62ce`, plan `pl-cb3e`).** A warren can fan runs across a pool of `burrow serve` workers on separate hosts; each worker stays stateful (per-host SQLite + sandboxes) and is reached over plain HTTP behind operator-supplied TLS termination on a VPC-private network. The operational surface lives in [DEPLOY.md](DEPLOY.md) ("Multi-worker topology"): `serve --bind-host` to expose the listener (non-loopback bind refuses to start when `--no-auth` is set), shared `BURROW_API_TOKEN` bearer auth across the pool, `POST /admin/drain` to quiesce a worker for rolling restarts (gates `POST /burrows` and `POST /burrows/:id/runs` with 503 `worker_draining` while in-flight runs continue to terminal state). Warren-side wiring (worker pool, placement, fan-out reads) lives in the warren repo; from burrow's side the contract is just bind-host + drain + the existing HTTP surface.
