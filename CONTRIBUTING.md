...
Thanks for your interest in contributing to Burrow! This guide covers everything you need to get started.
...
2. **Clone** your fork locally:
   ```bash
   git clone https://github.com/<your-username>/burrow.git
   cd burrow
   ```
...
bun test src/sandbox.test.ts               # Run a single test file
...
Burrow is a strict TypeScript project that runs directly on Bun (no build step).
...
- Use Bun built-in APIs where possible: `bun:sqlite` for the session store, `Bun.spawn` for sandbox processes, `Bun.file` / `Bun.write` for file I/O
...
- Sandbox primitives may be stubbed at the boundary (e.g., a fake `SandboxProvider`) but the layers above must run real code paths
- Clean up in `afterEach`
- Tests are colocated with source files: `src/sandbox.test.ts` alongside `src/sandbox.ts`
...
describe("my-feature", () => {
  ...
  beforeEach(async () => {
    testDir = await mkdtemp(join(tmpdir(), "burrow-test-"));
  });
...
```
fix: close inbox FD on burrow destroy
feat: add --network=none policy shortcut
docs: document burrow.toml schema
```
...
Use [GitHub Issues](https://github.com/jayminwest/burrow/issues) for bug reports and feature requests. For security vulnerabilities, see [SECURITY.md](SECURITY.md).
